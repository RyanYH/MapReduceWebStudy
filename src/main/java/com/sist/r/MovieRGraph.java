...
public class MovieRGraph {
	public void rGraph() {
		try {
			...
			rc.voidEval(
					"png(\"/home/sist/bigdataStudy/.metadata/.plugins/org.eclipse.wst.server.core/tmp0/wtpwebapps/MapReduceWebProject/movie/feel.png\",width=600,height=400)");
	...
	public List<FeelVO> movieFeelData() {
		...
		try {
			...
			rc.close();
			/*
			 * int max=0; for(int i=0;i<count.length;i++) { if(max<count[i])
			 * max=count[i]; }
			 */
